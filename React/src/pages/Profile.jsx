...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const myPosts = (postsResponse.data || []).filter(
          (post) => post.userId === decodedToken.id
        );
  ...
  if (errorUser) {
    return <div className="alert alert-danger text-center">{errorUser}</div>;
  }
    ...
    <div className="container py-4">
      <div className="row g-4">
        ...
        <div className="col-lg-8">
          ...
          {errorPosts && (
            <div className="alert alert-warning">{errorPosts}</div>
          )}
              ...
              {userPosts.map((post) => (
                ...
                >
                  <div className="d-flex w-100 justify-content-between">
                    <h5 className="mb-1 text-wrap-fix">{post.title}</h5>
