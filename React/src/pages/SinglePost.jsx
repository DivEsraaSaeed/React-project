...
export default function SinglePost() {
      ...
      try {
        ...
        setError(
          err.response?.data?.message ||
            "Failed to load the post. It might have been deleted or the link is incorrect."
        );
      } finally {
        setLoading(false);
      }
  ...
  if (error) {
    return (
      <div className="alert alert-danger text-center" role="alert">
        {error}{" "}
        <Link to="/posts" className="alert-link">
          Go back to posts
        </Link>
      </div>
    );
  ...
  return (
    <div className="card single-post-page">
      {notification && (
        <div
          className="alert alert-success text-center mb-0 border-bottom"
          role="alert"
      ...
      <div className="card-header d-flex justify-content-between align-items-center">
        <h2
          className="single-post-title mb-0 text-wrap-fix text-white"
        ...
        {isOwner && (
          <div className="btn-group">
            <Link
              ...
              className="btn btn-sm btn-outline-info me-3 rounded"
            >
      ...
      <div className="card-body">
          ...
          <img
            src={
              ...
              `https://ui-avatars.com/api/?name=${encodeURIComponent(
                post.user?.name || post.user?.username || "A"
              )}&background=0D6EFD&color=FFF&size=40&rounded=true`
            }
        ...
        <div
          className="single-post-content mt-4"
        ...
        {post.sections && post.sections.length > 0 && (
          <div className="mt-5 single-post-sections">
            ...
            {post.sections.map(
              (
                section // Use section.id if available and unique
              ) => (
                <div
